...
import SummaryItems from '../SummaryItems.svelte';

describe('SummaryItems', () => {
  it('should display the amount of items in the cart', () => {
    const { getByText } = render(SummaryItems, { props: { subTotal: 234.56, quantity: 5 }});
  ...
  it('should display the gross sub total items in the cart', () => {
    const { getByTestId } = render(SummaryItems, { props: { subTotal: 234.56, quantity: 5 }});
